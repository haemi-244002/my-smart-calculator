...
# ---------------------------------------------------------
    ...
    with st.form("schedule_form", clear_on_submit=True):
        ...
        with col2:
            task_input = st.text_input("📝 일정 내용 입력", placeholder="샘플이름/ 분석")
    ...
    st.divider()
    st.subheader("📌 해야 할 일 (To-Do)")
    
    # 💡 [버그 수정된 부분] 원래의 인덱스(고유 번호)를 유지한 채로 날짜 정렬만 수행!
    todo_df = df[df["완료여부"] == False].sort_values(by="날짜")

    if todo_df.empty:
        st.info("현재 남은 일정이 없습니다. 훌륭해요! 👏")
    else:
        ...
        if st.button("✅ 체크한 일정 목록에서 지우기"):
            # 이제 고유 번호가 원본 파일과 완벽하게 1:1로 매칭되므로 여러 개 동시 삭제 가능!
