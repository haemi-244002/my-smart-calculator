...
# 1. 페이지 기본 설정
st.set_page_config(page_title="나만의 만능 앱", layout="wide")

# --- 🚀 핵심: 구글 시트 연동 설정 ---
@st.cache_resource
def init_connection():
    # secrets.toml 에 숨겨둔 열쇠 정보 가져오기
    credentials_dict = json.loads(st.secrets["google_secret"])
    gc = gspread.service_account_from_dict(credentials_dict)
    # 아까 만든 구글 시트 이름 정확히 입력
    sh = gc.open("만능앱") 
    return sh.sheet1
...
def get_data():
    ...
    if not records: # 시트가 비어있을 경우
        return pd.DataFrame(columns=["날짜", "일정 내용", "완료여부"])
    df = pd.DataFrame(records)
    # 구글 시트의 문자열을 파이썬이 이해할 수 있는 True/False 로 변환
    df["완료여부"] = df["완료여부"].apply(lambda x: True if str(x).upper() == "TRUE" else False)
    # 날짜 데이터 형식 맞추기
    df["날짜"] = pd.to_datetime(df["날짜"]).dt.date
    return df

# 2. 왼쪽 사이드바 메뉴 만들기
st.sidebar.title("🧮 만능 앱 메뉴")
...
# ---------------------------------------------------------
    ...
    if st.button("습도 계산하기", key="btn_rh"):
        ...
        st.success("계산 완료!")
        st.info(f"🌡️ **입력 조건:** {T} ℃ / {RH}% RH\n- **포화 수증기압:** {P_sat_mmHg:.2f} mmHg\n- **현재 수증기압:** {P_act_mmHg:.2f} mmHg\n- **절대습도:** {AH:.2f} g/m³")
...
# ---------------------------------------------------------
    ...
    if st.button("스케줄 계산하기", key="btn_calcination"):
        if target_temp <= start_temp:
            ...
            st.success("계산 완료!")
            st.info(f"⏱️ **총 소요 시간:** {total_hours_display}시간 {total_mins_display}분\n🕒 **작동 종료 예정 시각:** {end_datetime.strftime('%Y년 %m월 %d일 %p %I시 %M분').replace('AM', '오전').replace('PM', '오후')}")

# ---------------------------------------------------------
# [기능 4] 내 일정 스케줄러 (구글 시트 연동)
# ---------------------------------------------------------
elif menu == "📅 분석 스케줄러":
    st.title("📅 내 분석 스케줄러")
    ...
    with st.form("schedule_form", clear_on_submit=True):
        ...
        with col2:
            task_input = st.text_input("📝 샘플이름", placeholder="예: 분석기기 / 샘플이름")
        ...
        if submitted:
            if task_input.strip() == "":
                ...
                # 구글 시트 맨 아래에 한 줄 추가 (문자열로 변환해서 저장)
                ws.append_row([str(date_input), task_input, "FALSE"])
                st.success("구글 시트에 일정이 성공적으로 추가되었습니다!")
                st.rerun()

    st.divider()

    tab1, tab2 = st.tabs(["📌 해야 할 일 (To-Do)", "📦 완료된 일정 보관함 (Done)"])

    with tab1:
        todo_df = df[df["완료여부"] == False].sort_values(by="날짜")

        if todo_df.empty:
            st.info("현재 남은 분석이 없습니다. 훌륭해요! 👏")
        else:
            edited_todo = st.data_editor(
                todo_df,
                column_config={
                    "완료여부": st.column_config.CheckboxColumn("끝냈나요?", default=False),
    ...
    with tab2:
        ...
        if done_df.empty:
            st.info("아직 완료된 일정이 없습니다.")
        else:
            st.success(f"🎉 지금까지 총 {len(done_df)}개의 일정을 완료하셨습니다!")
            st.dataframe(done_df[["날짜", "일정 내용"]], hide_index=True, use_container_width=True)
            
            st.divider()
            if st.button("🗑️ 보관함 기록 구글 시트에서도 영구 삭제하기"):
                df = df[df["완료여부"] == False]
